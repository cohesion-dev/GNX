	...
	for pageIdx, aiPage := range output.StoryboardPages {
		...
		for panelIdx, aiPanel := range aiPage.Panels {
			...
			for segIdx, aiSegment := range aiPanel.SourceTextSegments {
				...
				go s.generateSegmentTTS(segment.ID, segment.Text, segment.VoiceType, segment.SpeedRatio)
			}
...
func (s *ComicService) generateSegmentTTS(segmentID uint, text, voiceType string, speedRatio float64) {
	ctx := context.Background()
